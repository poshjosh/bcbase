 ...
 * 
 */
public class FileBasedCollection<E extends Serializable & Comparable<E>> 
        extends AbstractCollection<E> implements Closeable {
     ...
     */
    public FileBasedCollection(String saveToDir) {
        this(saveToDir, CHUNK_SIZE);
    }
     ...
     * @param chunkSize The size of each chunk.
     */
    public FileBasedCollection(String saveToDir, int chunkSize) {
        this(saveToDir, UUID.randomUUID().toString(), true, chunkSize);
    }
     ...
     * @param chunkSize The size of each chunk.
     */
    public FileBasedCollection(String saveToDir, String fileId, boolean deleteFileOnClose, int chunkSize) {
            this.store = new Store<>(saveToDir, fileId, deleteFileOnClose);
            this.store.getReader().readChunk().ifPresent((loadedList) -> {
                size = loadedList.size();
                System.out.println("XXXXXXXXXXXXXXXXXX Found size: " + size);
            });
...
//    }
    ...
    private static class Store<E extends Serializable> implements Closeable {
            ...
            Store(String dir, String stamp, boolean deleteFileOnClose) {
                    try {
                            file = createTmpFileForFileBasedCollection(dir, stamp, deleteFileOnClose);
            ...
            private File createTmpFileForFileBasedCollection(String dir, String stamp, boolean deleteFileOnClose) {
                    String uniqueFileName = "FileBasedCollection-";
                    String extension = ".bin";
                    File file = new File(dir, uniqueFileName + stamp + extension);
System.out.println("XXXXXXXXXXXXXXXX File: " + file);                    
...
System.out.println("XXXXXXXXXXXXXXXX Deleting: " + file);                                
                                file.delete();
                            }
            ...
            void write(ArrayList<E> chunk) {
                    try {
System.out.println("XXXXXXXXXXXXXXXXX To write: " + chunk);                                
                            ArrayList<E> output = new ArrayList<>();
                            this.getReader().readChunk().ifPresent((loadedList) -> {
System.out.println("XXXXXXXXXXXXXXXXX Loaded: " + loadedList);                                
                                output.addAll(loadedList);
                            });
                            output.addAll(chunk);
System.out.println("XXXXXXXXXXXXXXXXX Writing: " + output);                                
            ...
            ChunkReader<E> getReader() {
                    try {
                            ...
                            ChunkReader<E> reader = new ChunkReader<>(new ObjectInputStream(new BufferedInputStream(new FileInputStream(file))));
                            return reader;
                    } catch (IOException e) {
                            throw new RuntimeException(e);
                    }
...
System.out.println("XXXXXXXXXXXX Read: " + chunk);                            
